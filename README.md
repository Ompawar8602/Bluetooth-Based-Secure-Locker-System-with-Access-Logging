...
| ⏰ | **Alarm System** | Configurable time-based alarm with buzzer alert, merged in from a companion project |
...
```
   📱 Phone (Bluetooth)                🔢 Keypad
          │                                │
          ▼                                ▼
   ┌──────────────┐   Level-1 OK   ┌──────────────┐
   │  HC-05 UART1 │ ─────────────► │ Level-2 Entry │
   └──────────────┘                └──────┬───────┘
                                           │ match
                                           ▼
                                  ┌─────────────────┐
   ...
   🛡️ Tamper Switch ──► EINT ──► LCD Alert + Buzzer + Log
   🧑‍💻 Admin Button ──► EINT ──► CLK / Alarm / Password Menu
   🕒 RTC ──► Timestamps every event ──► UART0 Audit Log
```
